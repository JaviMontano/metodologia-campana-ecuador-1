# Forecast Model -- MetodologIA Bootcamp Quito 2026

| Campo | Valor |
|-------|-------|
| Versión | 2.0 |
| Fecha | 2026-04-01 |
| Fuente | content.json v2.0 |
| Marca | MetodologIA |
| Clasificacion | Indicativo, sujeto a consistencia de adopción |

---

## 1. Resumen de proyección a 24 meses

| Periodo | Ingresos | Costos | Neto |
|---------|----------|--------|------|
| Ano 1 | USD 136,049 | USD 9,756 | USD 126,293 [DOC] |
| Ano 2 | USD 434,900 | USD 15,000 | USD 419,900 [DOC] |
| **24 meses** | **USD 570,949** | **USD 24,756** | **USD 546,193** |

---

## 2. Forecast por segmento

### 2.1 B2C (CAO + Empowerment + Assistants)

**Y1: USD 86,849 | Y2: USD 150,000** [DOC]

#### Pipeline B2C -- Ano 1

| Ciclo | Mes | Producto | Precio | Estudiantes est. | Ingreso |
|-------|-----|----------|--------|-------------------|---------|
| 1 | Abril | CAO Believers | USD 1,490 | 8-10 | USD 11,920-14,900 [INFERENCIA] |
| 2 | Mayo | CAO Open + Empowerment | USD 1,490-1,990 | 10-12 | USD 14,900-23,880 [INFERENCIA] |
| 3 | Julio | CAO Academic | USD 1,990 | 10-15 | USD 19,900-29,850 [INFERENCIA] |
| 4 | Sept | CAO Academic | USD 1,990 | 10-15 | USD 19,900-29,850 [INFERENCIA] |
| 5 | Oct | Empowerment + Assistants | USD 347-1,490 | 15-20 | Variable [INFERENCIA] |
| 6 | Nov-Dic | CAO Academic cierre ano | USD 1,990 | 8-10 | USD 15,920-19,900 [INFERENCIA] |

Total planificado: 6 ciclos CAO en Y1 [DOC]. El mix de Assistants (USD 347) aumenta volumen y alimenta el funnel hacia CAO.

#### Supuestos B2C

- Conversión taller gratuito a CAO: 15-25% [SUPUESTO]
- Conversión Assistants a CAO/Empowerment: 10-15% [SUPUESTO]
- Retencion para segundo producto: 30-40% [SUPUESTO]
- Efecto Ambassador en Y2: multiplica alcance 2-3x [INFERENCIA]

### 2.2 B2B (Grupos corporativos)

**Y1: USD 13,200 | Y2: USD 98,400** [DOC]

#### Pipeline B2B

| Periodo | Cierres | Ticket promedio | Ingreso |
|---------|---------|-----------------|---------|
| Y1 Q1-Q3 | 0 | -- | Pipeline building [DOC] |
| Y1 Q4 | 1 | USD 13,200 | USD 13,200 [DOC] |
| Y2 Q1 | 1-2 | USD 16,400 | USD 16,400-32,800 [INFERENCIA] |
| Y2 Q2-Q4 | 4-5 | USD 16,400 | USD 65,600-82,000 [INFERENCIA] |

Total Y2: 6 cierres corporativos [DOC].

#### Supuestos B2B

- Ciclo de venta B2B: 3-6 meses [SUPUESTO]
- Tamano de grupo corporativo: 8-15 personas [SUPUESTO]
- Primer cierre esperado Q4 2026 despues de que los resultados B2C generen casos de estudio [INFERENCIA]
- La presencia de Ambassadors dentro de empresas acelera el pipeline B2B [INFERENCIA]

### 2.3 Cobrand (B2B2B)

**Y1: USD 36,000 | Y2: USD 96,000** [DOC]

#### Pipeline Cobrand

| Periodo | Partners | Ingreso por partner | Total |
|---------|----------|---------------------|-------|
| Y1 (cierre Nov) | 1 | USD 36,000 | USD 36,000 [DOC] |
| Y2 | 3-4 | USD 24,000-32,000 | USD 96,000 [DOC] |

#### Supuestos Cobrand

- Partner ideal: consultora, gremio empresarial, o universidad regional [INFERENCIA]
- El partner co-marca y co-comercializa; MetodologIA provee IP y coaching [DOC]
- Ciclo de negociacion: 4-6 meses [SUPUESTO]
- El primer partner se cierra en noviembre Y1 despues de 2-3 ciclos CAO exitosos como prueba de concepto [INFERENCIA]

### 2.4 White Label

**Y1: USD 0 | Y2: USD 90,500** [DOC]

#### Pipeline White Label

| Periodo | Estado | Detalle |
|---------|--------|---------|
| Y1 Q1-Q3 | Sin actividad | Foco en B2C y validación [DOC] |
| Y1 Q4 | Conversaciones iniciales | Post resultados CAO + Cobrand [DOC] |
| Y2 Q1-Q2 | Negociacion | 1-2 prospectos en pipeline [INFERENCIA] |
| Y2 Q3-Q4 | Cierres | USD 90,500 acumulado [DOC] |

#### Supuestos White Label

- Requiere IP documentada y probada (minimo 4 ciclos CAO) [SUPUESTO]
- Ticket alto, ciclo largo (6-9 meses) [SUPUESTO]
- Target: instituciones educativas regionales, corporativos multinacionales con presencia LatAm [INFERENCIA]
- Ingreso por licencia incluye fee inicial + royalty recurrente [POR CONFIRMAR]

---

## 3. Desglose mensual estimado -- Ano 1

| Mes | B2C | B2B | Cobrand | White Label | Total |
|-----|-----|-----|---------|-------------|-------|
| Abr | 11,920 | 0 | 0 | 0 | 11,920 [INFERENCIA] |
| May | 14,900 | 0 | 0 | 0 | 14,900 [INFERENCIA] |
| Jun | 5,200 | 0 | 0 | 0 | 5,200 [INFERENCIA] |
| Jul | 15,920 | 0 | 0 | 0 | 15,920 [INFERENCIA] |
| Ago | 8,500 | 0 | 0 | 0 | 8,500 [INFERENCIA] |
| Sep | 12,400 | 0 | 0 | 0 | 12,400 [INFERENCIA] |
| Oct | 6,009 | 3,200 | 0 | 0 | 9,209 [INFERENCIA] |
| Nov | 5,500 | 5,000 | 18,000 | 0 | 28,500 [INFERENCIA] |
| Dic | 6,500 | 5,000 | 18,000 | 0 | 29,500 [INFERENCIA] |
| **Total** | **86,849** | **13,200** | **36,000** | **0** | **136,049** |

Nota: Los montos mensuales son estimados para ilustrar la estacionalidad. El total por segmento coincide con los datos de content.json v2.0 [DOC]. La distribucion mensual es [INFERENCIA] basada en el calendario de ciclos.

---

## 4. Calendario de productos

| Fecha | Evento | Impacto en forecast |
|-------|--------|---------------------|
| Abril 2026 (Semana Santa) | Taller gratuito = Dia 1 | Generacion de leads, primer funnel [DOC] |
| Abril 2026 | CAO Believers -- primera cohorte | Primeros ingresos, validación de precio y formato [DOC] |
| Mayo 2026 | CAO Open (mercado abierto) + Empowerment | Expansion B2C, segundo producto [DOC] |
| Julio 2026 | CAO Academic -- inicio calendario académico | Mayor volumen, precio premium (USD 1,990) [DOC] |
| Q4 2026 | Primer cierre B2B + primer Cobrand | Diversificacion de ingresos [DOC] |
| Q1-Q2 2027 | Ambassador flywheel activo + White Label pipeline | Escala sin incremento proporciónal de costo [INFERENCIA] |

---

## 5. Ambassador Flywheel Model

El flywheel de Ambassadors es el motor de crecimiento organico de MetodologIA [DOC].

### Mecanica

```
Taller gratuito (funnel)
    |
    v
CAO Believers / Open / Academic (conversión 15-25%)
    |
    v
33 graduados CAO Y1 [INFERENCIA]
    |
    v
15-20 Ambassadors activos (50-60% de graduados) [INFERENCIA]
    |
    v
2-3 referidos por Ambassador [DOC]
    |
    v
30-60 leads calificados adicionales
    |
    v
15-25% conversión = 5-15 nuevos estudiantes
    |
    v
Ciclo se repite con base ampliada
```

### Proyeccion del flywheel

| Metrica | Y1 | Y2 |
|---------|----|----|
| Graduados CAO acumulados | 33 | 80-100 [INFERENCIA] |
| Ambassadors activos | 15-20 | 40-60 [INFERENCIA] |
| Referidos por Ambassador | 2-3 | 2-3 [DOC] |
| Leads generados por flywheel | 30-60 | 80-180 [INFERENCIA] |
| Conversión a pago | 5-15 | 20-45 [INFERENCIA] |

El descuento Ambassador (50% off) reduce el ticket promedio pero incrementa volumen y genera efecto de red [DOC]. Los Ambassadors dentro de empresas son el canal principal para abrir pipeline B2B [INFERENCIA].

---

## 6. Sensibilidad del forecast

### Escenario conservador (70% de base)

| Segmento | Y1 | Y2 |
|----------|----|----|
| B2C | 60,794 | 105,000 |
| B2B | 9,240 | 68,880 |
| Cobrand | 25,200 | 67,200 |
| White Label | 0 | 63,350 |
| **Total** | **95,234** | **304,430** |

### Escenario base (100%)

| Segmento | Y1 | Y2 |
|----------|----|----|
| B2C | 86,849 | 150,000 |
| B2B | 13,200 | 98,400 |
| Cobrand | 36,000 | 96,000 |
| White Label | 0 | 90,500 |
| **Total** | **136,049** | **434,900** |

### Escenario optimista (130% de base)

| Segmento | Y1 | Y2 |
|----------|----|----|
| B2C | 112,904 | 195,000 |
| B2B | 17,160 | 127,920 |
| Cobrand | 46,800 | 124,800 |
| White Label | 0 | 117,650 |
| **Total** | **176,864** | **565,370** |

Todos los escenarios son [INFERENCIA] basados en variaciones porcentuales sobre los datos base [DOC].

---

## 7. Metricas de control

| Metrica | Target Y1 | Target Y2 | Frecuencia de revision |
|---------|-----------|-----------|------------------------|
| Estudiantes CAO activos | 33+ | 80-100 | Mensual |
| NPS post-programa | >70 | >75 | Por cohorte |
| Tasa de conversión taller a CAO | >15% | >20% | Por ciclo |
| Ambassadors activos | 15-20 | 40-60 | Trimestral |
| Referidos por Ambassador | 2-3 | 2-3 | Trimestral |
| Cierres B2B | 1 | 6 | Trimestral |
| Pipeline B2B (valor) | 3x de target | 3x de target | Mensual |
| Burn rate | <USD 813/mes | <USD 1,250/mes | Mensual |

---

## Sensitivity: 3 Escenarios de Revenue Total

| Variable | Pesimista (70%) | Base (100%) | Optimista (130%) |
|----------|----------------|-------------|-------------------|
| Revenue Y1 | $95,234 | $136,049 | $176,864 |
| Revenue Y2 | $304,430 | $434,900 | $565,370 |
| Revenue 24mo | $399,664 | $570,949 | $742,234 |
| Net Y1 | $85,478 | $126,293 | $167,108 |
| Net Y2 | $289,430 | $419,900 | $550,370 |
| Break-even | Mes 1-2 | Mes 1 | Mes 1 |

**Supuesto critico:** Pesimista asume 30% menos cierres (4 CAO ciclos en vez de 6, 0 B2B Y1). Optimista asume 30% mas cierres + ambassador flywheel acelerado. [SUPUESTO]

---

## Cash Flow Timing

| Mes | Ingresos esperados | Fuente | Acumulado |
|-----|-------------------|--------|-----------|
| Abril | $7,450 | 5 Creyentes x $1,490 | $7,450 |
| Mayo | $19,900 | 10 CAO abierto x $1,990 | $27,350 |
| Mayo | $8,940 | 6 Empoderamiento x $1,490 | $36,290 |
| Julio | $9,950 | 5 CAO Académico | $46,240 |
| Q4 | $13,200 | 1 B2B closing | $59,440 |
| Nov | $36,000 | 1 Cobrand closing | $95,440 |

**Riesgo de cash flow:** Cobrand y B2B se cierran tarde (Q4). Los primeros 6 meses dependen 100% de B2C. Si B2C falla, no hay runway para B2B. [INFERENCIA]

---

## Estacionalidad Ecuador

| Periodo | Efecto | Impacto en ventas | Mitigacion |
|---------|--------|--------------------|-----------|
| Enero-Febrero | Vacaciones, presupuestos nuevos | Bajo (-30%) | No programar lanzamientos |
| Marzo-Abril | Pre-Semana Santa, presupuesto aprobado | Alto (+20%) | Timing ideal para Creyentes |
| Mayo-Junio | Estable | Base | CAO abierto + Empoderamiento |
| Julio-Agosto | Feriado agosto, vacacones sierra | Medio (-15%) | Académico con descuento early bird |
| Sept-Oct | Regreso a actividades, Q4 planning | Alto (+15%) | B2B y Cobrand closings |
| Nov-Dic | Cierre fiscal, presupuesto sobrante | Alto (+25%) | Push de B2B, White Label |

[INFERENCIA: basado en patrones de compra B2B Ecuador]

---

> **Innovation Mode**: Proyecciones indicativas. Los escenarios de sensibilidad no constituyen promesas de rendimiento. No es asesoria financiera.

*Construido por profesionales, potenciado por la red agéntica de MetodologIA.*
